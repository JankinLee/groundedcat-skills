...
def extract_novel_info(novel_dir: Path) -> dict:
    ...
    # 提取世界观（优先读取02文件，若存在03文件则追加）
    worldbuilding_parts = []
    worldbuilding_path_02 = novel_dir / '02-项目管理.md'
    if worldbuilding_path_02.exists():
        with open(worldbuilding_path_02, 'r', encoding='utf-8') as f:
            worldbuilding_parts.append(f.read())
    
    worldbuilding_path_03 = novel_dir / '03-世界观与时间线.md'
    if worldbuilding_path_03.exists():
        with open(worldbuilding_path_03, 'r', encoding='utf-8') as f:
            worldbuilding_parts.append(f.read())
    
    if worldbuilding_parts:
